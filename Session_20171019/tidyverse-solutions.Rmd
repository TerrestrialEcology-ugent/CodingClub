...
plant_size_tidy <- plant_size %>% spread(key=part, value= length) %>% separate(leaf, into= c("leaf_length","leaf_width"), sep="/")
...
```{r}
nr_seedlings_tidy3.4.1 <- nr_seedlings_tidy %>% group_by(plant, julian_day, inflorescence) %>% summarise(nr_seedlings= sum(nr_seedlings))

nr_seedlings_tidy3.4.2 <- nr_seedlings_tidy3.4.1 %>% group_by(inflorescence) %>% summarise(mean_nr_seedlings= mean(nr_seedlings), sd_nr_seedlings= sd(nr_seedlings))

nr_seedlings_tidy3.4.2 <- nr_seedlings_tidy3.4.1 %>% group_by(inflorescence) %>% summarise(total_nr_seedlings= sum(nr_seedlings))


```

```{r}
#nr_seedlings_tidy$plant <- as.character(nr_seedlings_tidy$plant)
nr_seedlings_tidy2 <- nr_seedlings_tidy %>% group_by(plant, julian_day) %>% summarise(nr_seedlings= sum(nr_seedlings)) %>% group_by(plant) %>% mutate(cum_nr_seedlings = cumsum(nr_seedlings))
